...
func Test_backxtrapolate(t *testing.T) {
	type args struct {
		sequence []int
	}
	tests := []struct {
		...
		{name: "example 1", args: args{sequence: []int{0, 3, 6, 9, 12, 15}}, want: -3},
		{name: "example 2", args: args{sequence: []int{1, 3, 6, 10, 15, 21}}, want: 0},
		{name: "example 3", args: args{sequence: []int{10, 13, 16, 21, 30, 45}}, want: 5},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {

			s2 := repeatdiff(tt.args.sequence)
			if got := backxtrapolate(s2); got != tt.want {
				t.Errorf("extrapolate() = %v, want %v", got, tt.want)
			}
		})
	}
