...
func Test_findContiguousSum(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := findContiguousSum(tt.args.data, tt.args.target)

			if got[0] != tt.want {
				t.Errorf("findContiguousSum() got = %v, want %v", got[0], tt.want)
			}
			if got[1] != tt.want1 {
				t.Errorf("findContiguousSum() got1 = %v, want %v", got[1], tt.want1)
			}
		})
	}
}
